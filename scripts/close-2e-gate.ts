...
interface SmokeOgCardsEvidence {
  at: string;
  base: string;
  results: Array<{ name: string; ok: boolean; detail?: string }>;
}

/** Mirrors close-1f-gate.ts's findProductionSmokeEvidence() pattern: unlike
 * 2B-2D (code-only gates), this phase's roadmap exit criterion explicitly
 * requires a real deployed R2 bucket and live-verified image responses, so
 * the gate checks for pre-existing evidence rather than running the live
 * smoke suite itself (which needs a real deploy + card upload first). */
function findOgCardsSmokeEvidence(): SmokeOgCardsEvidence {
  const dir = resolve("evidence");
  if (!existsSync(dir)) {
    throw new Error(
      `No evidence/ directory found — run 'npm run smoke:og-cards -- --url ${SITE_URL}' against the live production Worker first.`,
    );
  }
  const candidates = readdirSync(dir)
    .filter((name) => name.startsWith("smoke-og-cards-") && name.endsWith(".json"))
    .map((name) => JSON.parse(readFileSync(resolve(dir, name), "utf8")) as SmokeOgCardsEvidence)
  ...
  if (!latest) {
    throw new Error(
      `No evidence/smoke-og-cards-*.json found for base=${SITE_URL} — run 'npm run smoke:og-cards -- --url ${SITE_URL}' first.`,
    );
  }
  const failed = latest.results.filter((r) => !r.ok);
  if (failed.length > 0) {
    throw new Error(
      `Most recent production OG card smoke evidence (${latest.at}) has ${failed.length} failing check(s): ` +
...
function main() {
  console.log("Closing Phase 2E gate…");
  ...
  console.log(`Verifying live OG card smoke evidence against ${SITE_URL}…`);
  const smokeEvidence = findOgCardsSmokeEvidence();
  console.log(
    `  found evidence at ${smokeEvidence.at}: ${smokeEvidence.results.length} checks, all passing`,
  );

  const decision = `# Phase 2E Decision Record
...
- Live OG card smoke evidence exists for \`${SITE_URL}\` with every check
  passing (found at ${smokeEvidence.at}), proving a real deployed R2 bucket
...
`;
  ...
  writeFileSync(resolve("docs/phase-2e-decision-record.md"), decision);
  console.log("Wrote docs/phase-2e-decision-record.md");
  console.log("Phase 2E gate closed.");
}
