# Phase 1F Decision Record

Generated: 2026-08-07T03:13:21.631Z
...
caching headers, a production deploy bound to `https://theallodium.org`, a
...
- `scripts/deploy-worker.ts` (mirrors `promote-snapshot.ts`'s shape) runs
  ...
  `20278462-99fb-405b-86bc-184c6f8895d5` at 2026-08-07T03:08:38.874Z.
...
## `theallodium.com` → `https://theallodium.org` redirect
...
`theallodium.com` zone: a 301 to `https://theallodium.org` with the original path and
...
unknown route 404ing, and — only when run against `https://theallodium.org` itself —
...
one against `https://theallodium.org` to have every check passing.
...
- Independent of Selis except when publishing a new snapshot: the deployed
  Worker at `https://theallodium.org` serves entirely from D1 with zero runtime calls
